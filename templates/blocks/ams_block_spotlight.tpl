...
<{if $block.showother}>
    <div class= "itemOtherArticles" style="clear:both;">
        <h3><{$smarty.const._AMS_MB_SPOT_OTHERARTICLES}></h3>
        <ul>
            <{foreach item=news from=$block.stories}>
				<{if $news.friendlyurl_enable != 1}>
                <li style="clear:both;"><a href="<{$xoops_url}>/modules/ams/article.php?storyid=<{$news.id}>"><{$news.title}></a> (<{$news.hits}>)</li>
				<{else}>
                <li style="clear:both;"><a href="<{$news.friendlyurl}>"><{$news.title}></a> (<{$news.hits}>)</li>
				<{/if}>
